...
const ManageHalls = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEditing) {
                await axios.put(`http://localhost:5000/api/admin/halls/${formData._id}`, formData, config);
                toast.success('Hall updated successfully');
            } else {
                await axios.post('http://localhost:5000/api/admin/halls', formData, config);
    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-black text-[#1e3a8a] uppercase tracking-tight">Examination Halls</h1>
                    <p className="text-sm font-medium text-gray-400 mt-1">{halls.length} halls available containing {halls.reduce((acc, h) => acc + parseInt(h.capacity || 0), 0)} seats</p>
                </div>
                <button
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className="bg-gray-50/50">
                            <tr>
                                <th className="px-8 py-5 text-left text-[11px] font-black text-gray-400 uppercase tracking-widest">Hall Name</th>
                                <th className="px-8 py-5 text-left text-[11px] font-black text-gray-400 uppercase tracking-widest">Building</th>
                                <th className="px-8 py-5 text-left text-[11px] font-black text-gray-400 uppercase tracking-widest">Capacity</th>
                                <th className="px-8 py-5 text-right text-[11px] font-black text-gray-400 uppercase tracking-widest">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-50">
                            {filteredHalls.map((hall) => (
                                <tr key={hall._id} className="hover:bg-blue-50/30 transition-colors group">
                                    <td className="px-8 py-5">
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 rounded-xl bg-purple-50 text-purple-600 flex items-center justify-center font-black text-sm uppercase">
                                                {hall.hallName.substring(0, 2)}
                                            </div>
                                            <p className="font-bold text-gray-800 text-sm">{hall.hallName}</p>
                                        </div>
                                    </td>
                                    <td className="px-8 py-5">
                                    ...
                                    <td className="px-8 py-5">
                                        <span className="inline-flex items-center px-2.5 py-1 rounded-lg bg-green-50 text-green-700 text-[10px] font-black uppercase tracking-wider">
                                            {hall.capacity} Seats
                                        </span>
                                    </td>
                        ...
                        <div className="p-8">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Hall Name / Number</label>
                                    <input
                                        type="text"
                                        required
                                ...
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Building / Block</label>
                                    <input
                                        type="text"
                                        required
                                ...
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Seating Capacity</label>
                                    <input
                                        type="number"
                                        required
                                        min="1"
